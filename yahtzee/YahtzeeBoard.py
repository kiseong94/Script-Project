...
class YahtzeeBoard:
    ...
    round = 0           # 13라운드를 제어
    roll = 0            # 각 라운드 마다 3번 굴리기 roll을 할 수 있음
    ...
    def initInterface(self):    # Yahtzee 보드 윈도우 생성
        ...
        for i in range(5):      # dice 버튼 5개 생성
            ...
            # 각각의 dice 버튼에 대한 이벤트 처리 dieListener 연결
    ...
    def rollDiceListener(self):  # rollDiceListener
        for i in range(5):
            #if (self.diceButtons[i]['bg'] != 'disable'):
    ...
    def diceListener(self, row):  # DiceListener
        if self.diceButtons[row]['bg'] == 'light gray':
            self.diceButtons[row]['bg'] = 'white smoke'
        else:
